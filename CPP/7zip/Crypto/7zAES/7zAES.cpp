...
#else
static bool GetAESLibPath(TCHAR *path)
{
  const char *p7zip_home_dir = getenv("P7ZIP_HOME_DIR");
  if (p7zip_home_dir == 0) p7zip_home_dir="./";
  CSysString tmp = CSysString(p7zip_home_dir) + (CSysString)(TEXT("Codecs")) + (CSysString)(TEXT(STRING_PATH_SEPARATOR)) + TEXT("AES.dll");
  lstrcpy(path,&tmp[0]);
  return true;
}
#endif
