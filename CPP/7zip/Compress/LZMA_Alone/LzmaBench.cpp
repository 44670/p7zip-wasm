...
class CRandomGeneratorLZMA
{
  UInt32 A1;
  UInt32 A2;
public:
  CRandomGeneratorLZMA() { Init(); }
...
{
  CRandomGeneratorLZMA *RG;
public:
  void Set(CRandomGeneratorLZMA *rg) { RG = rg; }
...
struct CEncoderInfo
{
  ...
  HRESULT Init(UInt32 dictionarySize, UInt32 numThreads, CRandomGeneratorLZMA *rg);
  HRESULT Encode();
  ...
  static DWORD WINAPI EncodeThreadFunction(void *param)
  {
    ...
    // FIXED alloca(encoder->AllocaSize);
  ...
  static DWORD WINAPI DecodeThreadFunction(void *param)
  {
    ...
    // FIXED alloca(decoder->AllocaSize);
...
HRESULT CEncoderInfo::Init(UInt32 dictionarySize, UInt32 numThreads, CRandomGeneratorLZMA *rgLoc)
{
  rg.Set(rgLoc);
...
static void RandGen(Byte *buf, UInt32 size, CRandomGeneratorLZMA &RG)
{
  for (UInt32 i = 0; i < size; i++)
    buf[i] = (Byte)RG.GetRnd();
}

static UInt32 RandGenCrc(Byte *buf, UInt32 size, CRandomGeneratorLZMA &RG)
{
  RandGen(buf, size, RG);
